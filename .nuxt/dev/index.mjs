...
const _inlineRuntimeConfig = {
  ...
  "apiKey": "ur1286854-cb371ebec0ca41c53cc894a8",
  "sitePassword": "",
  "siteSecretKey": "site-status",
  "icon": {
...
const signJwt = async (expiresIn = "30d") => {
  try {
    const token = await new jose.SignJWT({ user: "admin" }).setProtectedHeader({ alg: "HS256" }).setExpirationTime(expiresIn).sign(SECRET_KEY);
    return token;
  } catch (error) {
...
const check_post = defineEventHandler(
  async (event) => {
    try {
      const config = useRuntimeConfig();
...
const formatNumber = (num) => Math.floor(num * 100) / 100;

const formatSiteData = (data, dates) => {
  if (!(data == null ? void 0 : data.monitors)) return void 0;
  const { public: configPublic } = useRuntimeConfig();
  const { showLink } = configPublic;
  const sites = data.monitors;
  const formatData = sites == null ? void 0 : sites.map((site) => {
    var _a, _b, _c, _d;
    ...
    (_a = site == null ? void 0 : site.logs) == null ? void 0 : _a.forEach((log) => {
      if ((log == null ? void 0 : log.type) === 1 || (log == null ? void 0 : log.type) === 99) {
        const date = dayjs.unix(log == null ? void 0 : log.datetime).format("YYYYMMDD");
    ...
    return {
      id: site.id,
      name: (site == null ? void 0 : site.friendly_name) || "Unnamed site",
      url: showLink ? site == null ? void 0 : site.url : void 0,
      status: (_b = site == null ? void 0 : site.status) != null ? _b : 8,
      type: (_c = site == null ? void 0 : site.type) != null ? _c : 1,
      interval: (_d = site == null ? void 0 : site.interval) != null ? _d : 0,
      percent,
      days: dailyData == null ? void 0 : dailyData.reverse(),
      down: total
    };
  });
  return {
...
const getRanges = () => {
  try {
    ...
    const today = dayjs((/* @__PURE__ */ new Date()).setHours(0, 0, 0, 0));
...
const getMonitors_post = defineEventHandler(async (event) => {
  try {
    const config = useRuntimeConfig();
...
const getMonitors_post$1 = /*#__PURE__*/Object.freeze({
  __proto__: null,
  default: getMonitors_post
});

const logout_post = eventHandler(
  async (event) => {
    try {
      ...
      return {
        code: 500,
        message: error instanceof Error ? error.message : "Internal Server Error. Failed to logout."
      };
    }
  }
);
...
const verify_post = defineEventHandler(
  async (event) => {
    try {
      ...
      if (!(body == null ? void 0 : body.password) || (body == null ? void 0 : body.password.length) === 0) {
        throw new Error("Password is required");
      }
      const hashedPassword = SHA256(sitePassword).toString();
      if ((body == null ? void 0 : body.password) !== hashedPassword) {
        setResponseStatus(event, 401);
        return {
          code: 401,
